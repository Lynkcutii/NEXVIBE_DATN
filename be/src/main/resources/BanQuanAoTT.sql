...
-- 15. Bảng trung gian Khuyến mãi - Khách hàng
CREATE TABLE KhuyenMai_KhachHang (
                                     IdKM INT NOT NULL,
                                     IdKH INT NOT NULL,
                                     PRIMARY KEY (IdKM, IdKH),
                                     FOREIGN KEY (IdKM) REFERENCES KhuyenMai(IdKM),
                                     FOREIGN KEY (IdKH) REFERENCES KhachHang(IdKH)
);

-- 16. Voucher (ĐÃ SỬA: MucGiam từ DECIMAL(5,2) sang DECIMAL(15,2))
...
('SP001', N'Áo Thun Nam Basic', 1, 1, 1, GETDATE(), 100, 'https://images.pexels.com/photos/428340/pexels-photo-428340.jpeg', N'Áo thun nam cotton cao cấp, co giãn 4 chiều, thấm hút mồ hôi tốt, kiểu dáng basic dễ phối đồ.', 199000, 1),
('SP002', N'Quần Jogger Thể Thao', 2, 2, 2, GETDATE(), 80, 'https://images.pexels.com/photos/6311606/pexels-photo-6311606.jpeg', N'Quần jogger thể thao chất polyester thoáng mát, bo gấu cá tính, phù hợp tập gym và đi chơi.', 299000, 1),
('SP003', N'Áo Khoác Dù Nam', 3, 3, 3, GETDATE(), 60, 'https://images.pexels.com/photos/4041684/pexels-photo-4041684.jpeg', N'Áo khoác dù chống nước, nhẹ, phù hợp cả nam và nữ, phong cách streetwear hiện đại.', 399000, 1);
...
INSERT INTO KhuyenMai_KhachHang (IdKM, IdKH) VALUES
(1, 1), -- Khuyến mãi mùa hè áp dụng cho KH001
(1, 2), -- Khuyến mãi mùa hè áp dụng cho KH002
(2, 1); -- Khuyến mãi sinh nhật áp dụng cho KH001
...
-- Chi tiết hóa đơn
INSERT INTO HoaDonCT (IdSPCT, IdHD, SoLuong, DonGia, ThanhTien, NgayTao, NgaySua, IdPT) VALUES
(1, 1, 2, 199000, 398000, GETDATE(), GETDATE(), 1),
(2, 1, 1, 299000, 299000, GETDATE(), GETDATE(), 1),
(3, 2, 1, 399000, 399000, GETDATE(), GETDATE(), 2),
(1, 3, 4, 199000, 796000, DATEADD(DAY, -10, GETDATE()), DATEADD(DAY, -10, GETDATE()), 1),
(2, 3, 2, 299000, 598000, DATEADD(DAY, -10, GETDATE()), DATEADD(DAY, -10, GETDATE()), 1),
(3, 4, 2, 399000, 798000, DATEADD(DAY, -8, GETDATE()), DATEADD(DAY, -8, GETDATE()), 2),
(1, 5, 1, 199000, 199000, DATEADD(DAY, -6, GETDATE()), DATEADD(DAY, -6, GETDATE()), 3),
(2, 5, 1, 299000, 299000, DATEADD(DAY, -6, GETDATE()), DATEADD(DAY, -6, GETDATE()), 3),
(3, 5, 1, 399000, 399000, DATEADD(DAY, -6, GETDATE()), DATEADD(DAY, -6, GETDATE()), 3),
(1, 6, 2, 199000, 398000, DATEADD(DAY, -4, GETDATE()), DATEADD(DAY, -4, GETDATE()), 1),
(2, 6, 1, 299000, 299000, DATEADD(DAY, -4, GETDATE()), DATEADD(DAY, -4, GETDATE()), 1),
(3, 7, 5, 399000, 1995000, DATEADD(DAY, -2, GETDATE()), DATEADD(DAY, -2, GETDATE()), 2),
(1, 8, 1, 199000, 199000, DATEADD(DAY, -1, GETDATE()), DATEADD(DAY, -1, GETDATE()), 3);
